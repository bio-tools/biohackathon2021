...
# fetch top 100 bio.tools records for a topic:
topic2genders <- function(topic) {
  tools <- read_json(paste0('https://bio.tools/api/tool/?topic="', topic, '"&format=json&page=1'))$list
  
  for(i in 2:8) {tools <- c(tools, read_json(
    paste0('https://bio.tools/api/tool/?topic="',topic, '"&format=json&page=',i))$list)
  }
...
# Example 3: Map geographic distribution of contributers to bio.tools, by the
# email address of the first/primary credited person or team.

count <- read_json('https://bio.tools/api/t?format=json')$count
tools <- read_json('https://bio.tools/api/t?format=json&page=1')$list

for(i in 2:round(0.5+count/10)) {tools <- c(tools, read_json(
  paste0('https://bio.tools/api/t?format=json&page=',i))$list)
}
...
tools <- read_json('https://bio.tools/api/tool/?t&format=json&page=1')$list

for(i in 1:length(tool[[1]]$`function`)) {
  for(j in 1:length(tool[[1]]$`function`[[i]]$operation)) {
    G <- add.vertices(G, 1, 
                      type = 'operation',
                      term = tool[[1]]$`function`[[j]]$operation[[j]]$term,
                      uri = tool[[1]]$`function`[[j]]$operation[[j]]$uri)
  }
}
...
entry <- ''
for(i in 1:length(tools)) {
  if(length(tools[[i]]$`function`)>0) {
    for(j in 1:length(tools[[i]]$`function`)) {
      if(length(tools[[i]]$`function`[[j]]$operation)>0) {
        for(k in 1:length(tools[[i]]$`function`[[j]]$operation)) {
          entry <- paste(entry, paste0('"8x', gsub(' ', '9x', tools[[i]]$`function`[[j]]$operation[[k]]$term), '8x"'), sep=' ')
        }
      }
    }
  }
