...
def main():
    # Read the tools
    with open("Resources/Small_tools.json", "r", encoding="utf8") as f:
        tools = json.load(f)
...
def calculate_statistics(raw_tools: list):
    ...
    operation_stats = calculate_edam_operation_statistics(tools=tools)
...
def calculate_edam_operation_statistics(tools: list) -> dict:
    """
    Calculate the statistics for EDAM operations.
    ...
    statistics = defaultdict(
        lambda: {'strict_ids': set(), 'total_ids': set(), 'strict_count': 0, 'total_count': 0})
    # Get the index list
    index_list: dict = _get_index_list(term_type="operation")
